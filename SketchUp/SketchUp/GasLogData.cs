	...
	public class GasLogData
	{
		...
		public bool InsertOrUpdate(DBAccessManager _fox)
		{
			if (IsDataValid)
			{
				if (IsNewRecord)
				{
					_fox.ExecuteNonSelectStatement(String.Format(_insertSQL, SketchUpGlobals.LocalLib, SketchUpGlobals.LocalityPreFix, Record, Card, NbrGasFP));
				}
				else
				{
					// update logic
		...
		public static GasLogData GetGasLog(DBAccessManager fox, int recno, int card)
		{
			...
			glsql.Append(String.Format("from {0}.{1}gaslg where grecno = {2} and gdwell = {3} ", SketchUpGlobals.LocalLib, SketchUpGlobals.LocalityPreFix, recno, card));
